...
# Langfuse integration — logs every simulation trace automatically
import os
from pathlib import Path
try:
    import langfuse as _lf_module
    from dotenv import load_dotenv
    load_dotenv(Path(__file__).parent.parent.parent.parent.parent / "tau2-bench" / ".env")
...
def run_simulation(
    ...
    *,
    ...
    Example:
        ...
        user = UserSimulator(llm="openrouter/qwen/qwen-2.5-72b-instruct", instructions=task.user_scenario,
    ...
    # Log simulation trace to Langfuse
    if _LANGFUSE_ENABLED:
        try:
            _lf.create_event(
                name="tau2-bench-simulation",
                input={
                    "task_id": str(task.id),
                    "domain": str(domain),
                    "mode": str(mode)
                ...
                output={
                    ...
                    "passed": bool(reward_info.reward >= 0.5)
                },
                metadata={
                    "model": "openrouter/qwen/qwen-2.5-72b-instruct",
                    "source": "tau2-bench-harness"
                }
            )
            _lf.flush()
        except Exception as e:
            pass  # Never block simulation on Langfuse errors
